...
const releaseNotes: ReleaseNote[] = [
    {
        ...
        improvements: [
            '管理コンソールで登録ユーザー一覧を表形式で確認できるようにし、ユーザーごとのパスワード再設定と削除操作に対応しました。',
            'ユーザー削除は専用モーダルで確認入力（対象ユーザー名一致）を必須化し、削除導線を赤系で強調して誤操作しにくくしました。',
            '存在しないURLに対する404ページを追加し、`404 NOT FOUND` と猫のアスキーアートを表示するようにしました。',
        ],
        fixes: [
            '管理者以外には管理コンソールのルートを公開せず、`/admin` を直接開いた場合も通常の404と同じ扱いになるように修正しました。',
            '管理専用APIを非管理者から呼び出した場合に404を返し、管理機能の露出を抑えるように修正しました。',
            '削除モーダルの角丸・配色・通知導線を見直し、ダークトーン環境での視認性と一貫性を改善しました。',
        ],
    },
    {
        version: '1.11.5',
